...
{
  return [NSString stringWithFormat:@"%@ - \n\t barcode_number: %@; \n\t name: %@; \n\t images: %@; \n\t list: %@; \n\t brand: %@; \n\t item_description: %@; \n", [super description], _barcode_number, _name, _images, _list, _brand, _item_description];
}

- (NSUInteger)hash
{
  NSUInteger subhashes[] = {[_barcode_number hash], [_name hash], [_images hash], [_list hash], [_brand hash], [_item_description hash]};
...
- (BOOL)isEqual:(Item *)object
{
  ...
  return
    ...
    (_list == object->_list ? YES : [_list isEqual:object->_list]) &&
